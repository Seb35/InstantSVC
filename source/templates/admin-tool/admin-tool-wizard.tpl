...
{{elseif $step eq "finish"}}
  ...
  <p>
   F&uuml;r folgende Klassen und Methoden soll ein XML-RPC-Server erstellt werden.</p>
  <ul>
  {{foreach item=class key=name from=$classes name=classes}}
    <li>{{$name}}
    <ul>
  ...
  <p>
   F&uuml;r folgende Klassen und Methoden soll ein REST-Server erstellt werden.</p>
  <ul>
  {{foreach item=class key=name from=$classes name=classes}}
    <li>{{$name}}
    <ul>

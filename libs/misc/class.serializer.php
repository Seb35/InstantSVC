 ...
 * last changes: Stefan Marr, get values from object properties by reflection line:923
 */
    ...
    */
    function _serializeObject(&$object, $tagName = null, $attributes = array())
    {
        ...
        if (method_exists($object, '__sleep')) {
            ...
            //Change: Stefan Marr
            //get property values by reflection api, but only public and protected
            $obj = new ReflectionObject($object);
            foreach ($obj->getProperties() as $key => $value) {
                if (!$value->isPrivate()) {
                    $properties[$value->getName()] = $value->getValue($object);
                }
            }

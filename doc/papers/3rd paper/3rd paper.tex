...
%Stefan: Nach welchem Schema sind eigentlich diese Namen geordnet?
\author{
\begin{center}
...
% 
% The paper headers
\markboth{Free and Open Source Software Conference 2006}{Web Service Toolkit für PHP5}
...
Ab diesem Punkt wird die Extended Reflection API aktiv. Sie liefert nicht 
nur die Methodennamen, sondern Listen von \texttt{ExtReflectionMethod}-Objekten, 
an denen direkt Informationen zu Parametern und Rückgabewerten abgefragen werden können. 
Dies ist ein großer Vorteile für den \texttt{WSDLGenerator}, da er so nicht selbst 
die Klassen parsen muss, um die benötigten Informationen zu erhalten.
...
Liste der \texttt{ExtReflectionMethod}-Objekte und liefert eine gefilterte Liste zurück. Die
Information, ob eine Methode veröffentlicht werden soll oder nicht, findet es
...
Namen der erzeugten Typen in einer Liste und greift während 
der Verarbeitung auch auf diese zu.

Der Generator erzeugt ein DOM-Dokument, das die WSDL-Struktur enthält. Der Aufbau dieser 
Struktur beginnt mit dem \verb|Definition|-Element, in dem alle verwendeten Namespaces
definiert werden und das alle anderen Beschreibungselemente enthält. Dann folgt
die \verb|Types|-Sektion, die XML Schema Beschreibungen für alle komplexen Datentypen
enthält und, bei Verwendung einer Literal-Kombination, außerdem die Anfrage- und
Antwortnachrichten definiert. Eine Liste der Methoden ist indirekt durch den
\verb|Message|-Teil gegeben, bei dem für jede Methode ein Element für die Anfrage und
ein Element für die entsprechende Antwort vorhanden ist. Darauf folgt die
\verb|PortType|-Abteilung, in der die Methoden auf Operationen abgebildet werden.
Anschließend wird die Kombination aus Aufruf und Kodierung explizit in
der \verb|Binding|-Sektion angegeben und abschließend folgt das \verb|Service|-Element, bei dem
...
Original-Klasse mit den Argumenten aus dem vom SOAP-Server übergebenen Objekt.
...
Wrapping von Rückgabewerten. Die Original-Klasse muss dazu in keiner Weise
angepasst werden.
...
\begin{enumerate}
	\item Anmelden, sowie Verschlüsseln der Anmeldedaten auf Seite des Clients
...
%DONE: sind die drei Punkte vor #PasswordDigest ein Fehler?!?
%Falko: Micha sagt, der Standard definiert das als URI der auf Type enden muss.
%       Ist also ok.
\begin{figure}[!htb]
\centering
...
Object Model (DOM) basierende Parser bilden im Speicher die Baumstruktur, des XML-Dokuments 
nach und ermöglichen somit den wahlfreien Zugriff auf alle Elemente. Die andere
Möglichkeit sind sequentielle, Ereignis-getriebene SAX-Parser. Sie sind einfach beschrieben
...
Username Token Profile auf Serverseite erklärt wurden, soll nun ein
...
Der Extended Server übernimmt dabei die Kapselung des eigentlichen SOAP-Server
...
eine allgemeingültige Schnittstelle zu verwenden, unter anderem 
...
\begin{figure}[htb]
	\centering
		\framebox[\linewidth]{\includegraphics[width=0.98\linewidth]{pics/AdminTool.eps}}
	\caption{Grafische Oberfläche des Administrations-Tools}
	\label{fig:AdminTool}
\end{figure}
...
\subsection{Repräsentation der Datenbankentitäten}
...
%TODO: readjust this pagebreak after modififications of the document
\newpage
\setlength{\parindent}{0pt}
nem konkreten Projekt
...
\emph {Username Token Profile}: http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0.pdf, 2004
